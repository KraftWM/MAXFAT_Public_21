...
% Funktion errechnet Pz Woehlerlinie aus gemessenen Daten der
...
SA = SA(notnans);
N = N(notnans);
% Rausschmeißen aller Durchlaeufer
notdl = N < ND;
EA = EA(notdl);
SA = SA(notdl);
N = N(notdl);
...
% -------------------------------------------------------------------------
% Schleife ueber Datenpunkte
Weff = zeros(numdata,1);
for i = 1 : numdata
    % ... aktuelle Amplituden
    ea = EA(i);
    sa = SA(i);
    % ... Rissoeffnungsspannung
...
% -------------------------------------------------------------------------
% a/c Verhaeltnis bestimmen sodass a/c = konst, d(a/c)/dn = 0
...
% Funktion gibt risslaengenabhaengige Geometriefunktionen in abhaengigkeit des
% Risslaengenverhaeltnisses a/c und Geometrieverhaeltniss a/rho
%
% Geometriefunktionen sind fuer Oberflaechenriss unter Zugbeanspruchung 
% Risstiefe a=1, Zugspannung sx=1
% Punkt A - tiefster Punkt
% Punkt B - bei theta = 10°
% 
% Werte werden interpoliert aus Stuetzstellen berechnet von Hertel.
% Siehe Diss Hertl.
%
% INPUT:
% azc        - Verhaeltniss a zu c (Stuetzstellen 1,3/4,2/3,1/2,1/4)
% azr        - Verhaeltniss a zu rho (fiktiver Kerbradius) (Stuetzstellen 0,1/4,1/2,1,2,4)
%
% OUTPUT:
