...
while aktdat <= numink
    ...
    if sum(REF(:,1)) == 0 && aktdat == 2
        ...
        ZVAR(ntens+1:end,1:jj) = ZVAR0(ntens+1:end).* ones(1,length(1:jj));% Speichern alles plastische hat sich net verändert
    else
        % ... sonst
        ZVAR(:,1) = ZVAR0;
    end
    ...
    for ii = jj + 1 : space+1
        ...
        if sum(ukp) == 0 && sum(eck) == 0
            ...
            GHAT = + DEL .* PHAT ...                                               % Spannungen
                + diag(EPSE - REF(:,4))...                                      % Elastische Dehnungen
                + diag(ZVAR(ntens+1:2*ntens,ii-1) - REF(:,5));                  % Plastische Dehnungen
            ...
            [ZVAR(:,ii), dTSED] = newton_bisektion(...
                ...
                'Neuber',...                     % Definiert Näherungsverfahren
                maxiter,tol,alpha,...            % Iter optionen
                dEEPS,...                        % Startwert Dehungsinkrement
                ii ...
                );
            
        end % Ende Verzweigung an Umkehrpunkten
